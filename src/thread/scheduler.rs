...
extern "C" fn run_thunk(sched: uint, code: *mut (), env: *mut ()) -> ! {
    ...
    c(scheduler);
    not_reached!("didn't unschedule finished thread");
}
...
impl<'a> Scheduler<'a> {
  ...
  pub fn new<'a>() -> Scheduler<'a> {
    ...
    let mut s = Scheduler { queue: box list as Box<Deque<Tcb>> };
  ...
  fn new_tcb(&self, func: extern "C" fn() -> ()) -> Tcb {
    ...
    let p  = proc(scheduler: &mut Scheduler) {
        func();
